...
public class AdminDashboardEndpointsTests : IntegrationTestBase, IClassFixture<TestWebApplicationFactory>
{
    ...
    [Fact]
    public async Task Obter_DeveIgnorarAcoesAdminRecentesForaDaJanelaRecente()
    {
        using var adminClient = _factory.CreateClient();
        ...
        await using (var scope = _factory.Services.CreateAsyncScope())
        {
            ...
            context.AuditoriasAdminAcoes.Add(new AuditoriaAdminAcao
            {
                ...
                Entidade = "dashboard",
                EntidadeId = Guid.NewGuid(),
                Acao = "acao-antiga-recente",
                Descricao = "Acao antiga para acoes recentes",
                PayloadJson = "{}",
                DataCriacao = DateTime.UtcNow.AddDays(-5)
            });
        ...
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.NotNull(payload);
        Assert.Empty(payload!.AcoesAdminRecentes);
    }
    ...
    public async Task Obter_ComPresetPeriodo_DeveAplicarJanelasPadraoDoPreset()
    {
        ...
        Assert.False(string.IsNullOrWhiteSpace(payload.AcaoComparativaPrincipal));
        Assert.StartsWith("/admin", payload.LinkComparativoPrincipal, StringComparison.Ordinal);
